...
async function loadMovies() {
  ...
  try {
    ...
    // ✅ get watchlist for highlight (optional, safe)
    const user = JSON.parse(localStorage.getItem("user"));
    let watchlistIds = [];

    if(user){
      try{
        const res = await fetch(`http://localhost:3000/api/auth/watchlist/${user.email}`);
        const wlData = await res.json();
        watchlistIds = (wlData.watchlist || []).map(m => m.movieId);
      }catch(err){
      ...
      card.innerHTML = `
        <div class="movie-wrapper" onclick="openMovie(${movie.id})">
          ...
          <button class="watchlist-btn"
            onclick="event.stopPropagation(); addToWatchlist(${movie.id})"
            ${isAdded ? "style='background:green;color:white'" : ""}
          >
            ${isAdded ? "✔" : "+"}
          </button>
    ...
    container.innerHTML = `
      ...
      <div class="movie-detail">
        ...
        <div class="movie-text">
          ...
          <p class="meta">
            ⭐ ${movie.vote_average} • Release Date: ${movie.release_date}
          </p>
...
async function addToWatchlist(movieId){
  ...
  try{
    ...
    // 🔥 STEP 1: Fetch full movie details
    const resMovie = await fetch(`${BASE_URL}/movie/${movieId}?api_key=${API_KEY}`);
    ...
    // 🔥 STEP 2: Get user rating
    const ratings = JSON.parse(localStorage.getItem("ratings")) || {};
    const userRating = ratings[movie.id] || 0;

    // 🔥 STEP 3: Safe genres extraction
    const genres = movie.genres 
      ? movie.genres.map(g => g.id)
      : [];

    // 🔥 STEP 4: Clean movie object
    const movieData = {
      movieId: movie.id,
      title: movie.title,
    ...
    console.log("Sending movie:", movieData); // ✅ DEBUG
